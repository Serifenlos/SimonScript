...
/*=================================================================================*/
function convertProfile_metaSoftproof(){if(editXMP_3(),xmpMeta.doesPropertyExist(nsURI,"softproofProfil")&&xmpMeta.doesPropertyExist(nsURI,"softproofIntent")&&xmpMeta.doesPropertyExist(nsURI,"softproofTK")){var e=xmpMeta.getProperty(nsURI,"softproofProfil"),t=xmpMeta.getProperty(nsURI,"softproofIntent"),a=Boolean(xmpMeta.getProperty(nsURI,"softproofTK"));if("image"==t)t=Intent.PERCEPTUAL;else if("colorimetric"==t)t=Intent.RELATIVECOLORIMETRIC;else if("graphics"==t)t=Intent.SATURATION;else if("absColorimetric"==t)t=Intent.ABSOLUTECOLORIMETRIC;doc.convertProfile(e,t,a,!0)}else//convert by Hand
...
/*     $.writeln(isWoodwing);
        ...
        $.writeln(idDocName); */if(o){try{BridgeTalkMessage_openDocID(i,n,debug);// v2
}catch(e){debug&&alert("bt: "+e)}try{closeDoc(n);// v2
}catch(e){debug&&alert("closeDoc+: "+e)}
// v3
...
var s=r}else{var c=new Folder(e);c.exists||c.create();s=c+"/"+replace_RGB_to_RZ()+"."+saveFormat}try{a.layerSets.getByName("Freisteller")&&(o?app.activeDocument.suspendHistory("Freisteller: 2 Ebenen","freisteller_reduce2layers()"):(gotoLayer("Freisteller"),fixMask(getActiveLayerIndex(),1),duplicateLayerMaskAsAlpha(),setMaskVisibility(!1),selectLayers("selectAllLayers"),mergeLayers(),setBackTheLayerMask(),setMaskVisibility(!1)))}catch(e){flattenImage()}RemoveAlphaChannels(),convertProfile_metaSoftproof(),sharp(sharp_dialog?DialogModes.ALL:DialogModes.NO),a.bitsPerChannel!=BitsPerChannelType.EIGHT&&setBitDepth(8),clearAllGuides(),delMeta_3(["softproofProfil","softproofIntent","softproofTK","softproofGroup"]),delPath&&activeDocument.pathItems.removeAll();try{if(o&&a.artLayers.getByName("frei"))return void saveMultiformat(new File(s),"psd",!1,null,RZ_alphaChannels,!0)}catch(e){}try{if(a.artLayers.getByName("Freisteller")){setMaskVisibility(!0);var l=c+"/"+replace_RGB_to_RZ()+"-frei";savePSD_v2(new File(l),t,t,t,f),setMaskVisibility(!1)}}catch(e){}try{flattenImage(),saveMultiformat(new File(s),saveFormat,!1,RZ_qualityJPG,RZ_alphaChannels,RZ_withLayers)}catch(e){debug&&alert("save: "+e)}
// closeDoc(woodwing_file);
}function closeDoc(e){try{app.documents.getByName(decodeURI(e)).close(SaveOptions.DONOTSAVECHANGES)}catch(t){debug&&alert("closeDoc: "+e+" :--: "+t)}}function BridgeTalkMessage_openDocID(e,t,a){var o=new BridgeTalk;o.target="indesign",o.body=runID.toSource()+"('"+e+"','"+t+"','"+a+"');",o.onResult=function(e){},o.send(10)}function runID(e,t,a){try{if(function(e){for(var t=!1,a=0;a<app.documents.length;a++)if(-1!==app.documents[a].name.indexOf(e)){t=!0,app.activeDocument=app.documents[a];break}return t}(e))app.activeDocument.links.itemByName(t).editOriginal();else alert("kein Focus auf der Datei?")}catch(e){
/* if (_debug) { alert("runID: " + e); } */}return" "}
