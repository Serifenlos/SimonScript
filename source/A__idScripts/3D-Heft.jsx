...
var fileList = Folder_single.getFiles(/.+\.jpg$/i);
sort_pages(fileList);
...
function tryPS(outputFolder) {
    ...
    gFilesToSkip = Array("db", "xmp", "thm", "txt", "doc", "md0", "tb0", "adobebridgedb", "adobebridgedbt", "bc", "bct", ".DS_Store");
    ...
    function RunFolder() {
        ...
        var filesOpened = 0;
        var fileList = Folder_single.getFiles();
        alert(fileList);
        for (var i = 0; i < fileList.length; i++) {
            if (fileList[i] instanceof File && !fileList[i].hidden && !IsFileOneOfThese(fileList[i], gFilesToSkip)) {
                ...
                /* alert(fileList[i]); */
                if ((i + 1) & 1) {
                    chooseSite2("links", fileList[i]);
                } else {
                    chooseSite2("rechts", fileList[i]);

                    var thisFileName = GetFileNameOnly(fileList[i].name);
                    var thisPageNumber = getPageNumber(fileList[i].name);
                    saveCopy([i], thisFileName, thisPageNumber);
                }
                emptyProtocoll();
            }
        }
    ...
    function saveCopy(Folder_3D, nummer, thisFileName, thisPageNumber) {

        var OutputFile = new File(Folder_3D + "/" + thisPageNumber + "__" + thisFileName);
    ...
    function getPageNumber(myFileName) {
        ...
        var rightPageNumber = myString.replace(/^(\d{3})(-|_{2})(.+)/g, "$1");
        var leftPageNumber = rightPageNumber - 1;
        var leftPageNumber = ("000" + leftPageNumber).slice(-3);
        var myString = (leftPageNumber + "-" + rightPageNumber);
        return myString
    ...
    function chooseSite2(thisSite, thisFile) {
...
function runPS(Folder_3D, master, thisFile, thisSite, thisFileName, thisPageNumber) {
    ...
    function saveCopy(thisFileName, thisPageNumber) {
        var OutputFile = new File(Folder_3D + "/" + thisPageNumber + "__" + thisFileName);
        tiffSaveOptions = new TiffSaveOptions();
        tiffSaveOptions.embedColorProfile = true;
        tiffSaveOptions.alphaChannels = false;
    ...
    if (thisSite == "links") {
        ...
        saveCopy(thisFileName, thisPageNumber);
    };
