...
function SaveForWeb(_format, _path, _filename, _scale, _interlace, _convert2sRGB, _embedICC, _matte, _matteR, _matteG, _matteB, _embedMeta,_jpgQquality, _jpgOptimized,_jpgProgressive, _jpgBlur) {
	...
	function SaveForWeb_helperFilename(_format,_filename) {
		if(_format == "JPEG" || _format == "JPG") {var extension = "jpg"}
		else if (_format == "PNG8" || _format == "PN24") {var extension = "png"}
		/* TODO gif + webm */
	...
	if(_format == "JPEG") {
		...
		if(_jpgProgressive) {	/* 1 = NO progressive // 3 = YES progressive */
	...
	if(_scale) {	/* enter a percent value OR false */
	...
	if(_convert2sRGB) {		 /* "CHsR" in sRGB konvertieren // "CHDc" nicht */
		d2.putEnumerated(c2t("SWch"), c2t("STch"), c2t("CHsR"));
	}
	else {d2.putEnumerated(c2t("SWch"), c2t("STch"), c2t("CHDc"))};
...
/*	d2.putEnumerated(c2t("SWmd"), c2t("STmd"), c2t("MDNn")); //"MDNn" ohne Metadaten // "MDCp" Copyright // "MDCC" Copyright und Kontaktinfos // "MDAx" Alle außer Kamera-Info // "MDAl" Alles */
	...
	function SaveForWeb_helperMeta(_embedMeta) {  /*"MDNn" ohne Metadaten // "MDCp" Copyright // "MDCC" Copyright und Kontaktinfos // "MDAx" Alle außer Kamera-Info // "MDAl" Alles */
		if(_embedMeta == "Meta_no") {var _embedMeta_ph = "MDNn"}
		else if (_embedMeta == "Meta_Copyright" || _embedMeta == "Meta_c") {var _embedMeta_ph = "MDCp"}
		else if (_embedMeta == "Meta_CopyrightKontakt" || _embedMeta == "Meta_ck") {var _embedMeta_ph = "MDCC"}
		else if (_embedMeta == "Meta_all_noKameraInfo" || _embedMeta == "Meta_all_no") {var _embedMeta_ph = "MDAx"}
		else if (_embedMeta == "Meta_all") {var _embedMeta_ph = "MDAl"}
		else {var _embedMeta_ph = "MDNn"} 
